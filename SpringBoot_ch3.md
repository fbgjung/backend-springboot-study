...
http 통신이란 문서를 전달하는 stateless 방식으로 필요한 작업을 수행하고나면 연결을 끊는다.

웹서버 통신은 소켓 통신과 http 통신의 단점을 해결하는 방식이며 클라이언트가 하나의 서버에 연결되어 있는 상태이다.

**[2] 톰캣이란 무엇인가요?**


웹서버 : 웹서비스를 할 수 있도록 하는 어플리케이션. 정적 데이터를 처리하며, 동적 데이터 처리가 필요한 경우 웹 컨테이너에게 데이터 전달

톰캣 : 웹 컨테이너라고도 불리며, 동적 데이터를 처리함. jsp나 서블릿을 구동할 수 있는 환경을 제공함

**[3] 서블릿 객체의 생명주기가 궁금해요!**

최초 요청인 경우라면 메모리 로딩 > 객체 생성 > 초기화(init) > 서비스 수행

최초 요청이 아닌 경우라면 바로 서비스 수행

**[4] 웹 배포 서술자(web.xml)에 대해서 알려줘요!**

서블릿 컨텍스트의 초기 파라미터를 관리하고 세션의 유효시간을 설정하며, 서블릿/jsp에 대한 정의 및 매핑을 수행한다. 또한 mime type을 매핑하고 welcome file list를 설정한다.

**[5] 디스패처 서블릿이 무엇인가요?**

FrontController 패턴에서 새로운 요청이 생기는 경우 RequestDispatcher가 필요함. 이때, 수 많은 객체가 생성되며 해당 필터들을 직접 등록하거나 자동 등록 되도록 할 수 있음

**[6] 애플리케이션 컨텍스트란 무엇인가요?**

디스패처 서블릿에 의해 생성되는 수 많은 객체들이 관리되는 곳이며, 객체가 필요한 곳에서 애플리케이션 컨텍스트에 접근하여 필요한 객체를 가져욜 수 있음. 종류로는 root-applicationContext와 servlet-applicationContext가 있음

**[7] 스프링부트가 응답하는 방법이 궁금해요!**

request가 들어오면 디스패처 서블릿이 해당 request를 매핑할 핸들러를 탐색한다. 이후 해당 핸들러를 실행시킬 어댑터를 탐색하여, handle 즉, 컨트롤러를 수행한다. 컨트롤러는 db 등을 이용하여 비즈니스 로직을 처리하며, 반환값을 모델에 저장한다. 이때, 반환값이 null이라면 view name 또한 반환하지 않고 데이터 형태로 클라이언트에게 response된다. 반환값이 모델에 저장되었다면, view name을 어댑터에게 전달하고 어댑터는 이를 디스패처 서블릿에게, 디스패처 서블릿은 view resolver에게 view 객체를 전달한다. view resolver는 view 객체를 view에게 전달하고 디스패처 서블릿은 모델을 view에게 전달한다. 디스패처 서블릿은 view에게 페이지를 표시할 것을 요청하고 view는 클라이언트에게 response한다.

⇒ 디스패처 서블릿이 request를 받게 되면 컨트롤러를 통해 비즈니스 로직을 처리하여 모델과 뷰를 생성하고 뷰가 이를 response하는 형태이다. 응답값이 데이터인 경우에는 모델과 뷰가 생성되지 않고 converter를 이용해 값이 response된다.
