...
**영속성**<br>
: 데이터를 영구적으로 저장하는 성질<br><br>

**컨텍스트**<br>
: 대상에 대한 모든 정보<br><br>

**영속성 컨텍스트**<br>
  : 자바가 데이터베이스에 데이터를 저장하는것들에 대한 모든 것들을 알고 있는것<br>
  + 영속성 컨텍스트에 있는 데이터와 데이터베이스에 있는 데이터가 동기화됨<br><br><br>

[5] JPA는 DB와 OOP의 불일치성을 해결하기 위한 방법론을 제공한다 (DB는 객체저장 불가능)<br><br>

ex) 영속성 컨텍스트를 이용하여 자바에서 select문을 수행하는 예시<br>
  1) 자바에서 selec로 db의 데이터를 호출<br>
  2) 영속성 컨텍스트는 db로부터 해당 데이터를 자바 object 형태로 만듦<br>

ex2) 자바 쪽에서 데이터를 추가하게 되는 경우에도, 영속성 컨텍스트가 db에 형식에 맞게 업데이트함<br><br>

* 데이터베이스의 타입은 primitive type만을 가질 수 있다 (객체 불가능)<br>
  => ORM을 이용하면 해결<br>
  => JPA가 데이터를 자동으로 매핑해주기 때문<br>

  ex) DB<br>
    Team[ ID, NAME, YEAR ] / Player[ ID, NAME, TEAMID]<br>
    (TEAMID  -> TEAM의 ID를 참조함)<br><br>

  ex) Java<br>
    자바에서는 필드값으로 객체를 가질 수 있으나, DB에서는 불가능하다. (두번의 select 혹은 join을 통해서만 가능)<br><br>
...
* 자바에서의 상속, 결합, 연관관계를 DB에도 자동으로 반영함
...
=> DB의 Car 테이블에 EntityDate의 필드(칼럼)가 자동 추가됨



[7] 방언 처리가 용이하여 Migration하기 좋음. 유지보수에도 좋음

**데이터베이스의 종류와 관계없이 사용할 수 있다**
: JPA가 자바와 데이터베이스 사이에서 추상화 객체를 두기 때문에, 데이터베이스의
종류가 바뀌어도 사용하기 용이하다.


[8] JPA는 쉽지만 어렵다

* oop를 바탕으로 배우고 사용하는 데 쉽지만, 방대한 데이터를 처리하다 보면
어려움이 있을 수 있다.
