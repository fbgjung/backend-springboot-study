...
# 1️⃣4️⃣ 스프링부트가 응답(Response)하는 방법이 궁금해요!
## 3.8 요청 주소에 따른 적절한 컨트롤러로 요청 (Handler Mapping)
Get 요청  
해당 주소 요청이 오면 적절한 컨트롤러의 함수를 찾아서 실행한다.  

## 3.9 응답
HTML 파일을 응답할지 Data를 응답할지 결정해야 하는데 HTML 파일을 응답하게 되면 `ViewResolver`가 관여하게 된다.   
하지만 Data를 응답하게 되면 `MessageConverter`가 작동하게 되는데 메시지를 컨버팅할 때 기본 전략은 `json`이다.  


클라이언트에서 요청하기 전,  
톰캣 실행시 web.xml이 호출되고 DB 관련된 것들을 메모리에 띄운다.  
클라이언트 요청이 들어오면,  
DispacherServlet이 웹과 관련된 것들을 메모리에 띄우고 주소를 분배한다.  
응답할때,   
데이터로 리턴할지 HTMl 파일로 리턴할지 정한다.  
